    ...
    for (;;)
    {
    	if (cyhal_uart_getc(&cy_retarget_io_uart_obj, &uart_read_value, 1)
    	            == CY_RSLT_SUCCESS)
    	        {
    	            ...
    	            else if (uart_read_value == 'q')
    	            {
    	                printf("pick a frequensy\r\n");
    	                cyhal_pwm_set_duty_cycle(&pwm_led_control, 60.0f, 1);

    	            }
    	            else if (uart_read_value == 'w')
    	            {
    	            	cyhal_pwm_set_duty_cycle(&pwm_led_control, 50.0f, 5);
    	            }
    	            else if (uart_read_value == 'e')
    	             {
    	              cyhal_pwm_set_duty_cycle(&pwm_led_control, 50.0f, 10);
    	             }
...
void btn_itrp_init (void)
{
	  ...
	  /* Initialize the user button */
	   ...
	   cyhal_gpio_callback_data_t intr_callback = {gpio_interrupt_handler, NULL, NULL, CYBSP_USER_BTN1 };
	   cyhal_gpio_register_callback(CYBSP_USER_BTN1, &intr_callback);
    ...
    if(!isButt)
          {
    		...
    		cyhal_pwm_start(&pwm_led_control);
    		cyhal_uart_putc(&cy_retarget_io_uart_obj, 't');
    		isButt = true;
